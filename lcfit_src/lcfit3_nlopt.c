...
/** NLopt constraint function and its gradient for enforcing that \f$\theta_b < (c + m)/(c - m)\f$.
 *
 * NLopt expects constraint functions of the form \f$f_c(x) \leq 0\f$, so we use
 * \f[
 *   f_c(x) = \theta_b - \frac{c + m}{c - m}
 * \f]
 *
 * \param[in]  p     Number of model parameters.
 * \param[in]  x     Model parameters to evaluate.
 ...
 * \return Value of the constraint function at \c x.
 */
double lcfit3_cons_theta_b_nlopt(unsigned p, const double* x, double* grad, void* data)
{
    const double c = x[0];
    const double m = x[1];
    ...
    if (grad) {
        grad[0] = -1/(c - m) + (c + m)/pow(c - m, 2);
        grad[1] = -1/(c - m) - (c + m)/pow(c - m, 2);
        grad[2] = 1.0;
    }
