...
START_TEST(test_scale)
{
  ...
  /* Try scaling */
  const double scale = cm_scale_factor(t, l, c, m, r, b);

  /* Test LL at t */
  const double calc_ll = ll(t, c*scale, m*scale, r, b);
...
fail_unless_fit_improves(const double x_init[4], const double t[4], const double l[4])
{
  ...
  for(i = 0; i < 4; ++i) {
    ...
    double init_fit_ll = ll(t[i], x_init[0], x_init[1], x_init[2], x_init[3]),
           new_fit_ll = ll(t[i], x[0], x[1], x[2], x[3]);
