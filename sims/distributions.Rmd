...
lcfit commit: `r system("git rev-parse --short HEAD", intern = TRUE)`
...
lcfit.dists.evals.wide <- tbl_df(dcast(lcfit.dists.evals,
                                       node_id + n_sites + n_leaves + model + seed + rate.dist + rate + source_tree + method + t ~ key, 
                                       value.var = "ll"))

by.node <- group_by(lcfit.dists.evals.wide, node_id, n_sites, n_leaves, rate.dist,
                    model, seed, rate, source_tree, method)

lcfit.dists <- summarise(by.node,
                         kl = kl_log(bpp, normal),
                         hellinger = hellinger_log(bpp, normal)) %>%
               ungroup()
```
...
```{r}
plot.evals <- function(evals) {
    ggplot(evals, aes(x = t)) +
      geom_line(aes(y = bpp, color = "bpp_ll")) +
      geom_line(aes(y = normal, color = "fit_ll")) +
      my_theme +
      my_legend
}
```

```{r hellinger.extremes}

lcfit.h.ex <- rbind(filter(lcfit.dists, min_rank(desc(hellinger)) <= 5),
                    filter(lcfit.dists, min_rank(hellinger) <= 5))

for (i in order(lcfit.h.ex$h)) {
  evals <- inner_join(lcfit.dists.evals.wide, lcfit.h.ex[i,], by = join.keys)
  print(plot.evals(evals) +
        ggtitle(sprintf('Bio++ vs non-iterative, hellinger = %f',
                        lcfit.h.ex[i, 'hellinger'])))
}
```

```{r kl.extremes}
lcfit.kl.ex <- rbind(filter(lcfit.dists, min_rank(desc(kl)) <= 5),
                     filter(lcfit.dists, min_rank(kl) <= 5))

for (i in order(lcfit.kl.ex$kl)) {
  evals <- inner_join(lcfit.dists.evals.wide, lcfit.kl.ex[i,], by = join.keys)
  print(plot.evals(evals) +
        ggtitle(sprintf('Bio++ vs non-iterative lcfit, kl = %f',
                        lcfit.kl.ex[i, 'kl'])))
}
```
