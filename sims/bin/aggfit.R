...
##' Calculate error estimates various fitting techniques
##'
##' .. content for \details{} ..
...
calculate.model.errors <- function(node) {
    errs <- ldply(c(weighted="weighted", unweighted="unweighted", top4="top4"),
                 function(analysis) {
                     ...
                     # if lcfit failed, set all error estimates to -1
                     # This makes it easy to filter these results out later.
                     
                     df <- compare_bls(ll$ll, node$bls$bpp_ll)
                     if (ll$status == LCFIT_SUCCESS)
                         df <- -df
                     return(df)
                    ...
                    function(node) {
                        ...
                        tmp <- llply(0:4,
                                     function(nextra) {
                                         ...
                                         # use model is provided, otherwise us maxima model as determined from simulation
...
write.csv(aggdata, 'aggfit.csv', row.names=FALSE)
