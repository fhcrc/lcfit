...
theme_update(legend.position="bottom", legend.key=element_blank(),
             plot.title=element_text(rel(1.0)))
...
r_values <- c(0.1, exp(-1/3), exp(1/3), 10.0)
r_values <- c(0.1, 0.5, 1, 5.0)
b_values <- c(0.5, 1.0, 1.5, 2)
m_values <- c(100, 250, 500, 1000)
c_values <- c(500, 1000, 2000, 5000)

b_varying <- expand.grid(t=x, b=b_values)

p_b <- ggplot(b_varying, aes(x=t, y=bsm_loglike(t, 1500, 1000, 2.0, b), linetype=factor(b))) +
  geom_line() +
  linetype('b') +
  xlabel +
  ylabel +
  annotate(geom="text", label="c=1500,m=1000,r=2.0", x=1.4, y=-1690) +
  varying_title('b')
...
p_r <- ggplot(r_varying, aes(x=t, y=bsm_loglike(t, 1500, 1000, r, .5), linetype=factor(r))) +
  ...
  xlabel +
  ylabel +
  annotate(geom="text", label="c=1500,m=1000,b=0.5", x=1.4, y=-3750) +
  varying_title('r')

# Plot varying m values
m_varying <- expand.grid(t=x, m=m_values)
p_m <- ggplot(m_varying, aes(x=t, y=bsm_loglike(t, 1500, m, 1.0, .5), linetype=factor(m))) +
  geom_line() +
  linetype('m') +
  xlabel +
  ylabel +
  annotate(geom="text", label="c=1500,r=1.0,b=0.5", x=1.5, y=-2000) +
  varying_title('m')


c_varying <- expand.grid(t=x, c=c_values)
p_c <- ggplot(c_varying, aes(x=t, y=bsm_loglike(t, c, 1000, 1.0, .5), linetype=factor(c))) +
  geom_line() +
  linetype('c') +
  xlabel +
  ylabel +
  annotate(geom="text", label="m=1000,r=1.0,b=0.5", x=1.5, y=-2500) +
  varying_title('c')

svg("parameter_effects.svg", width=18.5, height=5)
grid.arrange(p_c, p_m, p_r, p_b, nrow=1)
dev.off()
